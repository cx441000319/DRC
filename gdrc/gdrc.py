...
import gym
import time
import spinup.algos.pytorch.gdrc.core as core
...
class GDRCBuffer:
    ...
    def finish_path_for_gdrc(self, last_val, start_idx, end_idx):
...
def gdrc(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0, 
        steps_per_epoch=4000, epochs=250, gamma=0.99, clip_ratio=0.2, pi_lr=3e-4,
        vf_lr=1e-3, train_pi_iters=80, train_v_iters=80, lam=0.97, max_ep_len=1000,
        target_kl=0.01, logger_kwargs=dict(), save_freq=10,
        gcm_mode = 0, prob = 1.0, noise_disc = 6, cont_mode = 0, sigma=0.0, num_of_n_o=9):
    ...
    buf = GDRCBuffer(obs_dim, act_dim, local_steps_per_epoch, gamma, lam)
    ...
    rp_list = []
    rp_optimizer_list = []
    chosen_list = np.array([0.] * num_of_n_o) # store the votes
    for i in range(2, num_of_n_o*2+2, 2):
        rp = build_reward_critic(obs_dim[0] * 2, num_outputs=i, hidden_size=256)
        rp_optimizer = Adam(rp.parameters(), lr=vf_lr)
        rp_list.append(rp)
        rp_optimizer_list.append(rp_optimizer)
    ...
    def compute_loss_r(rp, obs, obs_prime, obs_rew, num_outputs):

        l_r = (max_reward - min_reward) / num_outputs
        obs_rew_cat = torch.floor((obs_rew - min_reward) // l_r)
    ...
    # SlidingWindow: handling the reward range
    class SlidingWindowLiveStats:
    ...
    percentiles = SlidingWindowLiveStats(20 * steps_per_epoch) # the capacity is set to be 20*steps_per_epoch
    ...
    for epoch in range(epochs):
        ...
        for t in range(local_steps_per_epoch):
            ...
            # store the sample in the sliding window
            percentiles.add_sample(r)
        ...
        # retrieve the min_reward and max_reward from the sliding window
        min_reward, max_reward = percentiles.get_stats()
        ...
        # train the drc each by each
        for i in range(len(rp_list)):
            for _ in range(rew_NN_ite):
                rp_optimizer_list[i].zero_grad()
                loss_r = compute_loss_r(rp_list[i], obs, next_obss, obs_rew, i*2+2)
                loss_r.backward()
                rp_optimizer_list[i].step()


        # ---------------------------------------
        # gdrc predicting
        
        # record the ordinal cross-entropy of each drc
        ord_ce_list = []
        for i in range(1, num_of_n_o+1):
            r_pre = predict_reward(rp_list[i-1], obs, next_obss)

            num_outputs = 2*i
            l_r = (max_reward - min_reward) / num_outputs
            r_cat = torch.floor((obs_rew - min_reward) // l_r)

            r_cat_np = r_cat.numpy()
            # critical!
            r_cat_np = np.clip(r_cat_np, 0, num_outputs-1)
            r_cat_np = torch.from_numpy(r_cat_np).type(torch.int64)
            ...
            ord_ce_list.append(ord_ce)

        increasing_rate_vibrate_first=1
        previous_increasing_rate = ord_ce_list[0]
        
        # choose the drc where the increasing rate of ordinal CE increases
        for i in range(1, num_of_n_o):
            increasing_rate = ord_ce_list[i]-ord_ce_list[i-1]
            if previous_increasing_rate>=increasing_rate:
                previous_increasing_rate=increasing_rate
            elif previous_increasing_rate<increasing_rate and increasing_rate_vibrate_first==1:
                increasing_rate_vibrate_first=0
                epoch_chosen_index=i-1
                break
        
        # if the increasing rate of ordinal CE keeps decreasing, choose the last drc
        if increasing_rate_vibrate_first != 0:
            epoch_chosen_index=num_of_n_o-1

        print("num_outputs = "+str((epoch_chosen_index+1)*2) + " is chosen!")
        
        # every epoch, the number of votes are timed by a discount factor (0.9)
        chosen_list *= 0.9
        chosen_list[epoch_chosen_index] += 1.
        chosen_index = np.argmax(chosen_list) # the drc used for prediction in this epoch
        num_outputs = (chosen_index+1)*2

        print("num_outputs = "+str(num_outputs) + " is used for prediction!")

        # use the chosen drc for reward prediction
        r_pre = predict_reward(rp_list[chosen_index], obs, next_obss)
        ...
        # finish the path for gdrc
        sum_ep_len = 0
        for i in range(len(ep_lens)):
            buf.finish_path_for_gdrc(last_vals[i], sum_ep_len, sum_ep_len + ep_lens[i])    # compute the advantages and target returns
        ...
        # open_for_reward_str.write(reward_str)
        # reward_str = ''
    ...
    # metrics: performance
    folder_path = "performance/gdrc"
    if not os.path.exists(folder_path):
        os.makedirs(folder_path)
    test_episode_rewards = open("performance/gdrc/" + logger_kwargs['exp_name'] + "_s" + str(seed) + ".txt", "w")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--noise_disc', type=int, default=6)
    parser.add_argument('--cont_mode', type=int, default=0)
    parser.add_argument('--sigma', type=float, default=0.0)
    parser.add_argument('--num_of_n_o', type=int, default=9)
    ...
    gdrc(lambda : gym.make(args.env), actor_critic=core.MLPActorCritic,
