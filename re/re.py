...
import gym
import time
import spinup.algos.pytorch.re.core as core
...
class REBuffer:
    ...
    def __init__(self, obs_dim, act_dim, size, gamma=0.99, lam=0.95):
        ...
        self.obs_rew_buf = np.zeros(size, dtype=np.float32) # observed reward as the target for reward predicti
    ...
    def finish_path_for_re(self, last_val, start_idx, end_idx):
...
def re(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0, 
    ...
    buf = REBuffer(obs_dim, act_dim, local_steps_per_epoch, gamma, lam)

    # exp_env
    if re_lib.search('Hopper', logger_kwargs['exp_name']):
        known_r_min = 0.5
        known_r_max = 6.5
    elif re_lib.search('HalfCheetah', logger_kwargs['exp_name']):
        known_r_min = -5.
        known_r_max = 7.
    elif re_lib.search('Walker2d', logger_kwargs['exp_name']):
        known_r_min = -3.
        known_r_max = 8.
    elif re_lib.search('Reacher', logger_kwargs['exp_name']):
        known_r_min = -12.
        known_r_max = 0.

    # reward estimator definition
    ...
    def format_r_input(s, a):
        return torch.cat([s, a], dim=-1)
    
    def rp_forward(inputs):
        x = inputs
    ...
    def predict_reward(s, a):
        r_hat_input = format_r_input(s, a)
        return rp_forward(r_hat_input)

    r_hat_input_size = obs_dim[0] + act_dim[0]
    rp = build_reward_critic(r_hat_input_size, num_outputs=1, hidden_size=64)
    rp_optimizer = Adam(rp.parameters(), lr=vf_lr)

    def compute_loss_r(data):
        obs, act, obs_rew = data['obs'], data['act'], data['obs_rew']
        rs_hat = torch.squeeze(predict_reward(obs, act))
        return ((rs_hat - obs_rew)**2).mean()
    ...
    def update():
        ...
        # Reward Estimator (RE) training
        for i in range(train_v_iters):
            rp_optimizer.zero_grad()
            loss_r = compute_loss_r(data)
            loss_r.backward()
            rp_optimizer.step()
    ...
    for epoch in range(epochs):
        ...
        for t in range(local_steps_per_epoch):
            ...
            # RE predicting
            i_o = torch.from_numpy(o.astype(np.float32))
            i_a = torch.from_numpy(a.astype(np.float32))
            i_o.type(torch.DoubleTensor)
            i_a.type(torch.DoubleTensor)

            with torch.no_grad():
                r_hat = predict_reward(i_o, i_a)
            p_hat = min(100, epoch) / 100
            estimated_reward = (1 - p_hat) * r + p_hat * r_hat.data.cpu()

            # save and log
            buf.store(o, a, estimated_reward, r, v, logp)
        ...
        # finish the path for RE
        sum_ep_len = 0
        for i in range(len(ep_lens)):
            buf.finish_path_for_re(last_vals[i], sum_ep_len, sum_ep_len + ep_lens[i])    # compute the advantages and target returns
    ...
    # metrics: performance
    folder_path = "performance/re"
    if not os.path.exists(folder_path):
        os.makedirs(folder_path)
    test_episode_rewards = open("performance/re/" + logger_kwargs['exp_name'] + "_s" + str(seed) + ".txt", "w")
...
if __name__ == '__main__':
    ...
    re(lambda : gym.make(args.env), actor_critic=core.MLPActorCritic,
