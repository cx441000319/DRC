...
def ppo(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0, 
        ...
        mode = 0, gcm_mode = 0, prob = .0, noise_disc = 10, cont_mode = 0, sigma=0.0):
    ...
    # set the reward range for GCM peturbation models
    ...
    # SR_W method
    if mode == 1:
        l_r = (known_r_max - known_r_min) / noise_disc
        mmat = np.expand_dims(np.arange(known_r_min+l_r, known_r_max+1e-1, l_r, dtype=float), axis=1)
        phi = np.linalg.inv(alphas).dot(mmat)
    ...
    for epoch in range(epochs):
        for t in range(local_steps_per_epoch):
            ...
            # SR_W method
            if mode == 1:
                cat_per_sample = int((r - known_r_min) // l_r)
                if cat_per_sample<0:
                    cat_per_sample=0
                elif cat_per_sample > noise_disc-1:
                    cat_per_sample = noise_disc-1
                r = phi[cat_per_sample, 0]
    ...
    # logout evaluation performance
    if mode != 1:
        folder_path = "performance/ppo"
        if not os.path.exists(folder_path):
            os.makedirs(folder_path)
        test_episode_rewards = open("performance/ppo/" + logger_kwargs['exp_name'] + "_s" + str(seed) + ".txt", "w")
    else:
        folder_path = "performance/sr_w"
        if not os.path.exists(folder_path):
            os.makedirs(folder_path)
        test_episode_rewards = open("performance/sr_w/" + logger_kwargs['exp_name'] + "_s" + str(seed) + ".txt", "w")
